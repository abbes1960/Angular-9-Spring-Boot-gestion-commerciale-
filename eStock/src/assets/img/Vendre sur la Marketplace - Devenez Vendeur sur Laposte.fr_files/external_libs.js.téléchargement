var Utils = {
    ...
    writeTrackerPIXEL : function (trackerUrl) {
        var i = new Image(1,1);
        i.src = this.appendGDPRParams(trackerUrl);
    ...
    writeTrackerIFRAME : function (trackerUrl) {
        ...
        i.setAttribute('src',this.appendGDPRParams(trackerUrl));
    ...
    writeTrackerJAVASCRIPT : function (trackerUrl) {
        ...
        g.src = this.appendGDPRParams(trackerUrl);
  ...
  appendGDPRParams: function(url){
    ...
    if (this.gdpr_params === null) {
      var params = [], vars = document.location.search.substring(1).split('&');
